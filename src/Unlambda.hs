...
compile context x@(Var (Just _) _) = x
compile context x@(Var Nothing  v)
...
exists n (Var Nothing  _) = False
exists n (Var (Just m) _) = n == m
exists n (e  :$ e')       = exists n e || exists n e'
exists n (v' :^ e )       = exists (n + 1) e

liftIndexes :: Expr -> Expr
liftIndexes v@(Var Nothing _) = v
liftIndexes (Var (Just n) x)  = Var (Just $ n - 1) x
...
-- -- | λ式の中から変数を取り除く
-- resolve :: Expr -> Expr
-- resolve e@(Var Nothing  _) = k :$ e
-- resolve e@(Var (Just m) y)
--     -- ^x.x === i
--     | m == 0                   = i
--     -- ^x.y === `ky
--     | otherwise                = k :$ Var (Just $ m - 1) y
--
-- resolve e@(e' :$ e''@(Var m y))
--     -- ^x.M === `kM
--     | not (0 `isBoundedIn` e)  = k :$ e
--     -- ^x.`Mx === M
--     | m == Just 0              = e'
--     -- ^x.`MN === ``s ^x.M ^x.N
--     | otherwise                = s :$ resolve e' :$ resolve e''
...
-- isBoundedIn :: Index -> Expr -> Bool
-- n `isBoundedIn` Var Nothing  _ = False
-- n `isBoundedIn` Var (Just m) _ = n == m
-- n `isBoundedIn` (_ :^ e)       = (n + 1) `isBoundedIn` e
-- n `isBoundedIn` (e :$ e')      = n `isBoundedIn` e || n `isBoundedIn` e'

-- -- | 式の中に変数が自由変数として現れるかどうか判定する
...
-- n `isFreeIn` (e :$ e') = x `isFreeIn` e || x `isFreeIn` e'

--------------------------------------------------------------------------------

subst :: Context -> Expr -> Expr
subst context v@(Var Nothing x)
    | x `Map.member` context     = let f = context ! x
                                   in  subst context $ body f
    | otherwise                  = v
subst context v@(Var (Just _) _) = v
subst context (e :$ e')          = subst context e :$ subst context e'
subst context (v :^ e )          = v :^ subst context e

-- -- | 式に含まれる定義済み変数を再帰的に展開する
-- subst :: Context -> Expr -> Expr
